...
class SimpleStateMachine:
    def __init__(self):
        ...
        self.robot = transport_drive_motor_API.TransportDriveMotorAPI()
        ...
        self.target_distance_subscriber = rospy.Subscriber("target_distance", int, lambda: self.target_distance_changed)
        # Subscribe to the current distance from the target. For now, that's an IR value.
        self.current_distance_subscriber = rospy.Subscriber("range_data", int, lambda: self.current_distance_reading_changed)
    ...
    def current_distance_reading_changed(self, new_current_distance):
        self.currentDistance = new_current_distance
        # Data has changed! Tick the state machine!
        ssm.woodenStateMachine.tick()
